...
function testeDilithium() {
  async function test() {
  const kind = 2; // Dilithium2

  // gerar chaves
  const { publicKey, privateKey } = await generateDilithiumKeyPair(kind);
  ...
  // assinar
  const signature = await signWithDilithium(message, privateKey, kind);
  console.log("Signature:", Buffer.from(signature).toString("base64"));

  // verificar
  const isValid = await verifyWithDilithium(signature, message, publicKey, kind);
